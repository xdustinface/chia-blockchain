...
def benchmark_conversion_template(
    iterations: int,
...
class BenchmarkParameter:
    ...
    mode_parameter: Dict[Mode, Union[int, ConversionParameter]]
...
benchmark_parameter: Dict[Data, BenchmarkParameter] = {
    Data.benchmark: BenchmarkParameter(
        ...
        {
            Mode.creation: 1100,
            Mode.to_bytes: ConversionParameter(2200, to_bytes),
            Mode.from_bytes: ConversionParameter(3600, BenchmarkClass.from_bytes, to_bytes),
            Mode.to_json: ConversionParameter(1100, BenchmarkClass.to_json_dict),
            Mode.from_json: ConversionParameter(930, BenchmarkClass.from_json_dict, BenchmarkClass.to_json_dict),
        },
    ),
    Data.full_block: BenchmarkParameter(
        ...
        {
            Mode.creation: 85,
            Mode.to_bytes: ConversionParameter(9650, to_bytes),
            Mode.from_bytes: ConversionParameter(365, FullBlock.from_bytes, to_bytes),
            Mode.to_json: ConversionParameter(2400, FullBlock.to_json_dict),
            Mode.from_json: ConversionParameter(335, FullBlock.from_json_dict, FullBlock.to_json_dict),
        },
    ),
}
                ...
                if mode == Mode.all or current_mode == mode:
                    ...
                    for _ in range(max(1, runs)):
                        if current_mode == Mode.creation:
                            assert isinstance(mode_parameter, int)
                            iterations = mode_parameter * multiplier
                            duration = benchmark_object_creation(iterations, parameter.object_creation_cb)
                        else:
                            assert isinstance(mode_parameter, ConversionParameter)
                            assert mode_parameter.conversion_cb is not None
                            iterations = mode_parameter.iterations * multiplier
                            duration = benchmark_conversion_template(
                                iterations,
