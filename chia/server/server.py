...
class ChiaServer:
    ...
    def get_connections(
        self, node_type: Optional[NodeType] = None, direction: Optional[Direction] = None
    ) -> List[WSChiaConnection]:
        result = []
        for _, connection in self.all_connections.items():
            if (node_type is None or connection.connection_type == node_type) and (
                direction is None or connection.direction == direction
            ):
    ...
    def accept_inbound_connections(self, node_type: NodeType) -> bool:
        if not self._local_type == NodeType.FULL_NODE:
            return True
        inbound_count = len(self.get_connections(node_type, Direction.Inbound))
