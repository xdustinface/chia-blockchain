...
async def test_validate_blockchain_spend_reorg_coin(
    two_nodes: Tuple[FullNodeAPI, FullNodeAPI, ChiaServer, ChiaServer, BlockTools], softfork_height: uint32
) -> None:
    num_blocks = 10
    wallet_a = WALLET_A
    coinbase_puzzlehash = WALLET_A_PUZZLE_HASHES[0]
...
async def test_assert_my_coin_id(
    ...
    valid_spend_bundle = wallet_a.generate_signed_transaction(uint64(1000), receiver_puzzlehash, spend_coin, valid_dic)
