...
@pytest.mark.asyncio
async def test_block_tools_proofs_400(default_400_blocks, blockchain_constants):
...
@pytest.mark.asyncio
async def test_block_tools_proofs_1000(default_1000_blocks, blockchain_constants):
    vdf, proof = get_vdf_info_and_proof(
        blockchain_constants,
        ClassgroupElement.get_default_element(),
        blockchain_constants.GENESIS_CHALLENGE,
...
@pytest.mark.asyncio
async def test_block_tools_proofs(blockchain_constants):
    vdf, proof = get_vdf_info_and_proof(
        blockchain_constants,
        ClassgroupElement.get_default_element(),
        blockchain_constants.GENESIS_CHALLENGE,
...
@pytest.mark.asyncio
async def test_non_overflow_genesis(empty_blockchain, bt):
...
@pytest.mark.asyncio
async def test_overflow_genesis(empty_blockchain, bt):
    genesis = bt.get_consecutive_blocks(1, force_overflow=True)[0]
    await _validate_and_add_block(empty_blockchain, genesis)


@pytest.mark.asyncio
async def test_genesis_empty_slots(empty_blockchain, bt):
    genesis = bt.get_consecutive_blocks(1, force_overflow=False, skip_slots=30)[0]
    await _validate_and_add_block(empty_blockchain, genesis)


@pytest.mark.asyncio
async def test_overflow_genesis_empty_slots(empty_blockchain, bt):
    genesis = bt.get_consecutive_blocks(1, force_overflow=True, skip_slots=3)[0]
    await _validate_and_add_block(empty_blockchain, genesis)


@pytest.mark.asyncio
async def test_genesis_validate_1(empty_blockchain, bt):
    genesis = bt.get_consecutive_blocks(1, force_overflow=False)[0]
    bad_prev = bytes([1] * 32)
...
@pytest.mark.asyncio
async def test_long_chain(empty_blockchain, default_1000_blocks, consensus_mode: Mode):
    ...
    for block in blocks:
        if (
            ...
            # Also fails calling the outer methods, but potentially with a different error
            await _validate_and_add_block(empty_blockchain, block_bad_2, expected_result=AddBlockResult.INVALID_BLOCK)
            ...
            # Also fails calling the outer methods, but potentially with a different error
            await _validate_and_add_block(empty_blockchain, block_bad_3, expected_result=AddBlockResult.INVALID_BLOCK)
            ...
            # Also fails calling the outer methods, but potentially with a different error
            await _validate_and_add_block(empty_blockchain, block_bad_4, expected_result=AddBlockResult.INVALID_BLOCK)
        await _validate_and_add_block(empty_blockchain, block)
        log.info(
            f"Added block {block.height} total iters {block.total_iters} " f"new slot? {len(block.finished_sub_slots)}"
        )
    assert empty_blockchain.get_peak().height == len(blocks) - 1


@pytest.mark.asyncio
async def test_unfinished_blocks(empty_blockchain, softfork_height, bt):
...
@pytest.mark.asyncio
async def test_empty_genesis(empty_blockchain, bt):
    for block in bt.get_consecutive_blocks(2, skip_slots=3):
        await _validate_and_add_block(empty_blockchain, block)


@pytest.mark.asyncio
async def test_empty_slots_non_genesis(empty_blockchain, bt):
    blockchain = empty_blockchain
...
@pytest.mark.asyncio
async def test_one_sb_per_slot(empty_blockchain, bt):
    blockchain = empty_blockchain
    num_blocks = 20
    blocks = []
...
@pytest.mark.asyncio
async def test_all_overflow(empty_blockchain, bt):
    blockchain = empty_blockchain
    num_rounds = 5
    blocks = []
    num_blocks = 0
...
@pytest.mark.asyncio
async def test_unf_block_overflow(empty_blockchain, softfork_height, bt):
    blockchain = empty_blockchain
    ...
    while True:
        ...
        if sb_1.overflow and sb_2_next_ss:
            ...
            if block.transactions_generator is not None:
                ...
                block_bytes = bytes(unf)
                npc_result = await blockchain.run_generator(block_bytes, block_generator, height=softfork_height)
            validate_res = await blockchain.validate_unfinished_block(unf, npc_result, skip_overflow_ss_validation=True)
            assert validate_res.error is None
            return None
...
@pytest.mark.asyncio
async def test_one_sb_per_two_slots(empty_blockchain, bt):
    blockchain = empty_blockchain
    num_blocks = 20
    blocks = []
...
@pytest.mark.asyncio
async def test_one_sb_per_five_slots(empty_blockchain, bt):
    blockchain = empty_blockchain
    num_blocks = 10
    blocks = []
...
@pytest.mark.asyncio
async def test_basic_chain_overflow(empty_blockchain, bt):
...
@pytest.mark.asyncio
async def test_one_sb_per_two_slots_force_overflow(empty_blockchain, bt):
    blockchain = empty_blockchain
    num_blocks = 10
    blocks = []
    for i in range(num_blocks):
...
@pytest.mark.asyncio
async def test_invalid_prev(empty_blockchain, bt):
    # 1
...
@pytest.mark.asyncio
async def test_invalid_pospace(empty_blockchain, bt):
    # 2
    blocks = bt.get_consecutive_blocks(2, force_overflow=False)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_invalid_sub_slot_challenge_hash_genesis(empty_blockchain, bt):
    # 2a
...
@pytest.mark.asyncio
async def test_invalid_sub_slot_challenge_hash_non_genesis(empty_blockchain, bt):
    # 2b
...
@pytest.mark.asyncio
async def test_invalid_sub_slot_challenge_hash_empty_ss(empty_blockchain, bt):
    # 2c
    blocks = bt.get_consecutive_blocks(1, force_overflow=False, skip_slots=0)
...
@pytest.mark.asyncio
async def test_genesis_no_icc(empty_blockchain, bt):
    # 2d
    blocks = bt.get_consecutive_blocks(1, force_overflow=False, skip_slots=1)
    new_finished_ss = recursive_replace(
        blocks[0].finished_sub_slots[0],
...
async def do_test_invalid_icc_sub_slot_vdf(keychain, db_version, constants: ConsensusConstants):
    ...
    for block in blocks:
        if len(block.finished_sub_slots) > 0 and block.finished_sub_slots[-1].infused_challenge_chain is not None:
            ...
            new_finished_ss = recursive_replace(
                ...
                InfusedChallengeChainSubSlot(
                    replace(
                        block.finished_sub_slots[-1].infused_challenge_chain.infused_challenge_chain_end_of_slot_vdf,
...
@pytest.mark.asyncio
async def test_invalid_icc_sub_slot_vdf(db_version, blockchain_constants):
    with TempKeyring() as keychain:
        await do_test_invalid_icc_sub_slot_vdf(keychain, db_version, blockchain_constants)


@pytest.mark.asyncio
async def test_invalid_icc_into_cc(empty_blockchain, bt):
    blockchain = empty_blockchain
...
@pytest.mark.asyncio
async def test_empty_slot_no_ses(empty_blockchain, bt):
    # 2l
    blockchain = empty_blockchain
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_empty_sub_slots_epoch(empty_blockchain, default_400_blocks, bt):
    # 2m
...
@pytest.mark.asyncio
async def test_wrong_cc_hash_rc(empty_blockchain, bt):
    # 2o
    blockchain = empty_blockchain
...
@pytest.mark.asyncio
async def test_invalid_cc_sub_slot_vdf(empty_blockchain, bt):
    # 2q
...
@pytest.mark.asyncio
async def test_invalid_rc_sub_slot_vdf(empty_blockchain, bt):
    # 2p
    blocks: List[FullBlock] = []
...
@pytest.mark.asyncio
async def test_genesis_bad_deficit(empty_blockchain, bt):
    # 2r
...
@pytest.mark.asyncio
async def test_reset_deficit(empty_blockchain, bt):
    # 2s, 2t
    blockchain = empty_blockchain
...
@pytest.mark.asyncio
async def test_genesis_has_ses(empty_blockchain, bt):
    # 3a
    ...
    new_finished_ss = recursive_replace(
        new_finished_ss,
        "reward_chain",
        replace(new_finished_ss.reward_chain, challenge_chain_sub_slot_hash=new_finished_ss.challenge_chain.get_hash()),
    )
    block_bad = recursive_replace(block, "finished_sub_slots", [new_finished_ss] + block.finished_sub_slots[1:])
    with pytest.raises(AssertionError):
        # Fails pre validation
        await _validate_and_add_block(empty_blockchain, block_bad, expected_error=Err.INVALID_SUB_EPOCH_SUMMARY_HASH)


@pytest.mark.asyncio
async def test_no_ses_if_no_se(empty_blockchain, bt):
    # 3b
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_too_many_blocks(empty_blockchain):
    # 4: TODO
    pass


@pytest.mark.asyncio
async def test_bad_pos(empty_blockchain, bt):
    # 5
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_bad_signage_point_index(empty_blockchain, bt):
    # 6
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_sp_0_no_sp(empty_blockchain, bt):
    # 7
    blocks = []
    case_1, case_2 = False, False
    while not case_1 or not case_2:
...
@pytest.mark.asyncio
async def test_bad_rc_sp_vdf(empty_blockchain, bt):
    # 11
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_bad_rc_sp_sig(empty_blockchain, bt):
    # 12
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_bad_cc_sp_vdf(empty_blockchain, bt):
...
@pytest.mark.asyncio
async def test_bad_cc_sp_sig(empty_blockchain, bt):
    # 14
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
    block_bad = recursive_replace(blocks[-1], "reward_chain_block.challenge_chain_sp_signature", G2Element.generator())
    await _validate_and_add_block(empty_blockchain, block_bad, expected_error=Err.INVALID_CC_SIGNATURE)


@pytest.mark.asyncio
async def test_is_transaction_block(empty_blockchain):
    # 15: TODO
    pass


@pytest.mark.asyncio
async def test_bad_foliage_sb_sig(empty_blockchain, bt):
    # 16
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_bad_foliage_transaction_block_sig(empty_blockchain, bt):
    # 17
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_unfinished_reward_chain_sb_hash(empty_blockchain, bt):
    # 18
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_pool_target_height(empty_blockchain, bt):
    # 19
    blocks = bt.get_consecutive_blocks(3)
    await _validate_and_add_block(empty_blockchain, blocks[0])
    await _validate_and_add_block(empty_blockchain, blocks[1])
...
@pytest.mark.asyncio
async def test_pool_target_pre_farm(empty_blockchain, bt):
    # 20a
    blocks = bt.get_consecutive_blocks(1)
    block_bad: FullBlock = recursive_replace(
...
@pytest.mark.asyncio
async def test_pool_target_signature(empty_blockchain, bt):
    # 20b
...
@pytest.mark.asyncio
async def test_pool_target_contract(empty_blockchain, bt, consensus_mode: Mode):
...
@pytest.mark.asyncio
async def test_foliage_data_presence(empty_blockchain, bt):
    # 22
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
    case_1, case_2 = False, False
...
@pytest.mark.asyncio
async def test_foliage_transaction_block_hash(empty_blockchain, bt):
    # 23
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
    case_1, case_2 = False, False
    while not case_1 or not case_2:
        ...
        if blocks[-1].foliage_transaction_block is not None:
            ...
            await _validate_and_add_block(empty_blockchain, block_bad, expected_error=Err.INVALID_FOLIAGE_BLOCK_HASH)
            return None
        await _validate_and_add_block(empty_blockchain, blocks[-1])


@pytest.mark.asyncio
async def test_genesis_bad_prev_block(empty_blockchain, bt):
    # 24a
    blocks = bt.get_consecutive_blocks(1)
    block_bad: FullBlock = recursive_replace(
...
@pytest.mark.asyncio
async def test_bad_prev_block_non_genesis(empty_blockchain, bt):
    # 24b
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
    while True:
...
@pytest.mark.asyncio
async def test_bad_filter_hash(empty_blockchain, bt):
    # 25
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
    while True:
...
@pytest.mark.asyncio
async def test_bad_timestamp(bt):
    # 26
...
@pytest.mark.asyncio
async def test_height(empty_blockchain, bt):
    # 27
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_height_genesis(empty_blockchain, bt):
    # 27
    blocks = bt.get_consecutive_blocks(1)
...
@pytest.mark.asyncio
async def test_weight(empty_blockchain, bt):
    # 28
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_weight_genesis(empty_blockchain, bt):
    # 28
    blocks = bt.get_consecutive_blocks(1)
...
@pytest.mark.asyncio
async def test_bad_cc_ip_vdf(empty_blockchain, bt):
    # 29
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_bad_rc_ip_vdf(empty_blockchain, bt):
    # 30
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_bad_icc_ip_vdf(empty_blockchain, bt):
    # 31
    blocks = bt.get_consecutive_blocks(1)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_reward_block_hash(empty_blockchain, bt):
    # 32
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_reward_block_hash_2(empty_blockchain, bt):
    # 33
    blocks = bt.get_consecutive_blocks(1)
...
@pytest.mark.asyncio
async def test_pre_validation_fails_bad_blocks(empty_blockchain, bt):
    blocks = bt.get_consecutive_blocks(2)
    await _validate_and_add_block(empty_blockchain, blocks[0])
...
@pytest.mark.asyncio
async def test_pre_validation(empty_blockchain, default_1000_blocks, bt):
...
async def test_conditions(empty_blockchain, opcode, with_garbage, bt):
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
async def test_timelock_conditions(opcode, lock_value, expected, bt):
    async with make_empty_blockchain(bt.constants) as b:
        ...
        tx: SpendBundle = wt.generate_signed_transaction(10, wt.get_new_puzzlehash(), coin, condition_dic=conditions)
...
async def test_aggsig_garbage(empty_blockchain, opcode, with_garbage, expected, bt, consensus_mode: Mode):
...
async def test_ephemeral_timelock(opcode, lock_value, expected, with_garbage, bt):
...
@pytest.mark.asyncio
async def test_not_tx_block_but_has_data(empty_blockchain, bt):
    # 1
    blocks = bt.get_consecutive_blocks(1)
...
@pytest.mark.asyncio
async def test_tx_block_missing_data(empty_blockchain, bt):
    # 2
    b = empty_blockchain
...
@pytest.mark.asyncio
async def test_invalid_transactions_info_hash(empty_blockchain, bt):
    # 3
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(2, guarantee_transaction_block=True)
    await _validate_and_add_block(b, blocks[0])
...
@pytest.mark.asyncio
async def test_invalid_transactions_block_hash(empty_blockchain, bt):
    # 4
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(2, guarantee_transaction_block=True)
    await _validate_and_add_block(b, blocks[0])
...
@pytest.mark.asyncio
async def test_invalid_reward_claims(empty_blockchain, bt):
    # 5
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(2, guarantee_transaction_block=True)
    await _validate_and_add_block(b, blocks[0])
    ...
    block_2: FullBlock = recursive_replace(block, "transactions_info.reward_claims_incorporated", too_few_reward_claims)
...
@pytest.mark.asyncio
async def test_invalid_transactions_generator_hash(empty_blockchain, bt):
    # 7
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(2, guarantee_transaction_block=True)
    await _validate_and_add_block(b, blocks[0])
...
@pytest.mark.asyncio
async def test_invalid_transactions_ref_list(empty_blockchain, bt, consensus_mode: Mode):
...
@pytest.mark.asyncio
async def test_cost_exceeds_max(empty_blockchain, softfork_height, bt):
    # 7
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
@pytest.mark.asyncio
async def test_clvm_must_not_fail(empty_blockchain, bt):
    # 8
    pass


@pytest.mark.asyncio
async def test_invalid_cost_in_block(empty_blockchain, softfork_height, bt):
    # 9
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
@pytest.mark.asyncio
async def test_max_coin_amount(db_version, bt):
    # 10
...
@pytest.mark.asyncio
async def test_invalid_merkle_roots(empty_blockchain, bt):
    # 11
    blocks = bt.get_consecutive_blocks(
        3,
        guarantee_transaction_block=True,
...
@pytest.mark.asyncio
async def test_invalid_filter(empty_blockchain, bt):
    # 12
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
@pytest.mark.asyncio
async def test_duplicate_outputs(empty_blockchain, bt):
    # 13
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
@pytest.mark.asyncio
async def test_duplicate_removals(empty_blockchain, bt):
    # 14
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
@pytest.mark.asyncio
async def test_double_spent_in_coin_store(empty_blockchain, bt):
    # 15
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
@pytest.mark.asyncio
async def test_double_spent_in_reorg(empty_blockchain, bt):
    # 15
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
@pytest.mark.asyncio
async def test_minting_coin(empty_blockchain, bt):
    ...
    tx: SpendBundle = wt.generate_signed_transaction(10, wt.get_new_puzzlehash(), spend, condition_dic=condition_dict)
...
@pytest.mark.asyncio
async def test_max_coin_amount_fee():
    # 18 TODO: we can't create a block with such amounts due to uint64
    pass


@pytest.mark.asyncio
async def test_invalid_fees_in_block(empty_blockchain, bt):
    # 19
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
@pytest.mark.asyncio
async def test_invalid_agg_sig(empty_blockchain, bt):
    # 22
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
...
@pytest.mark.asyncio
async def test_basic_reorg(empty_blockchain, bt):
    b = empty_blockchain
...
@pytest.mark.asyncio
async def test_long_reorg(empty_blockchain, default_1500_blocks, test_long_reorg_blocks, bt):
...
@pytest.mark.asyncio
async def test_long_compact_blockchain(empty_blockchain, default_2000_blocks_compact):
    b = empty_blockchain
...
@pytest.mark.asyncio
async def test_reorg_from_genesis(empty_blockchain, bt):
    b = empty_blockchain
...
@pytest.mark.asyncio
async def test_reorg_transaction(empty_blockchain, bt):
    b = empty_blockchain
...
@pytest.mark.asyncio
async def test_get_header_blocks_in_range_tx_filter(empty_blockchain, bt):
    b = empty_blockchain
    blocks = bt.get_consecutive_blocks(
        3,
        guarantee_transaction_block=True,
...
@pytest.mark.asyncio
async def test_get_blocks_at(empty_blockchain, default_1000_blocks):
    b = empty_blockchain
    heights = []
