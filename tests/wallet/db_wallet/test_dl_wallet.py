...
@pytest.mark.asyncio
async def test_initial_creation(self_hostname: str, simulator_and_wallet: SimulatorsAndWallets, trusted: bool) -> None:
...
async def test_get_owned_singletons(
    self_hostname: str, simulator_and_wallet: SimulatorsAndWallets, trusted: bool
) -> None:
    full_nodes, wallets, _ = simulator_and_wallet
    full_node_api = full_nodes[0]
    full_node_server = full_node_api.server
...
@pytest.mark.asyncio
async def test_tracking_non_owned(self_hostname: str, two_wallet_nodes: SimulatorsAndWallets, trusted: bool) -> None:
...
@pytest.mark.asyncio
async def test_lifecycle(self_hostname: str, simulator_and_wallet: SimulatorsAndWallets, trusted: bool) -> None:
    full_nodes, wallets, _ = simulator_and_wallet
    full_node_api = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
...
@pytest.mark.asyncio
async def test_rebase(self_hostname: str, two_wallet_nodes: SimulatorsAndWallets, trusted: bool) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api = full_nodes[0]
    full_node_server = full_node_api.server
    wallet_node_0, server_0 = wallets[0]
    ...
    update_txs = await dl_wallet_0.create_update_state_spend(launcher_id, bytes32([0] * 32), fee=uint64(2000000000000))
